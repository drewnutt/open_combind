...
def test_coalesce_poses():
    ...
    sorted_poses = coalesce_poses(sort_files, sort_file)
    ...
    sorted_poses = coalesce_poses(sort_files, sort_files)
...
def test_write_poses():
    ...
    with fileinput.hook_compressed("open_combind/tests/test_write.sdf.gz") as f:
        supplier = ForwardSDMolSupplier("open_combind/tests/test_write.sdf.gz")
        count = 0
        for mol in supplier:
            count += 1
        assert count == 40
