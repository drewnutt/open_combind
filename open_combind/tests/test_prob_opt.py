...
#def create_ligand(rmsds, gscores, fps):
#        ligand_data = {'rmsds':rmsds,'gscores':gscores,'fps':fps}
#        return ligand_data

#def combine_ligands(ligand_dict):
#    # converts dict of dict from ligands in upper to ligands in lower
#    ligands = list(ligand_dict.keys())
#    new_dict = {key:dict() for key in ligand_dict[ligands[0]].keys()}
#    for lig,ligvals in ligand_dict.items():
#        for measure, vals in ligvals.items():
#            new_dict[measure][lig] = vals
#    return new_dict

#def basic_ps():
#        ligands = {'lig1': create_ligand([0, 1], [-2, -1.5], [{}, {(1, 23): 1.0}]),
#                           'lig2': create_ligand([0, 1], [-10, -3.5], [{(1, 23): 1.0}, {}])}
...
#        features = {'sb': [1]}
#        return PosePrediction(ligands.keys(), ['sb'], features, stats, 1.0)

#def two_interaction_ps():
#        ligands = {'lig1': create_ligand([0, 1], [-2, -1.5],
#                                         [{(2, 10): 0.0}, {(1, 23): 1.0, (2, 10): 1.0}]),
#                           'lig2': create_ligand([0, 1], [-10, -3.5],
#                                                 [{(1, 23): 1.0}, {(2, 10): 0.6}])}
#        stats = {'native': {'sb': DensityEstimate(domain = (0, 1)),
#                                                'hbond': DensityEstimate(domain = (0, 1))},
#                         'reference': {'sb': DensityEstimate(domain = (0, 1)),
...
#        return PosePrediction(ligands.keys(), ["sb","hbond"], features, stats, 1.0)
...
#        stats['reference']['sb'].fx = np.linspace(2, 0, 100)
#        features = {'sb': [1]}
#        return PosePrediction(ligands, None, stats, features, 3, 1.0)
